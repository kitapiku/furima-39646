...
RSpec.describe User, type: :model do
  ...
  describe '新規登録(本人確認情報)' do
    ...
    it 'お名前(全角)は全角（漢字・ひらがな・カタカナ）でないと登録できない' do
      ...
      @user.valid?
      expect(@user.errors.full_messages).to include("First name should be a full-width name", "Last name should be a full-width name")
    end
    ...
    it 'お名前カナ(全角)は全角（カタカナ）でないと登録できない' do
      ...
      @user.valid?
      expect(@user.errors.full_messages).to include("First kananame should be a full-width katakana name", "Last kananame should be a full-width katakana name")
    end
